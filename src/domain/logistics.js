...
// 各省/市 → 所属大区,用于就近匹配转运中心
const REGION_RE = [
  [/上海|浙江|杭州|宁波|温州|嘉兴|湖州|绍兴|金华|衢州|舟山|台州|丽水|江苏|南京|无锡|徐州|常州|苏州|南通|连云港|淮安|盐城|扬州|镇江|泰州|宿迁|安徽|合肥|芜湖|蚌埠|淮南|马鞍山|淮北|铜陵|安庆|黄山|阜阳|宿州|滁州|六安|宣城|池州|亳州|义乌|昆山/, [
    '杭州转运中心', '义乌华东分拨中心', '昆山华东转运中心'
  ]],
  [/广东|广州|深圳|珠海|汕头|佛山|韶关|河源|梅州|惠州|汕尾|东莞|中山|江门|阳江|湛江|茂名|肇庆|清远|潮州|揭阳|云浮|广西|南宁|柳州|桂林|梧州|北海|防城港|钦州|贵港|玉林|百色|贺州|河池|来宾|崇左|海南|海口|三亚/, [
    '广州华南分拨中心', '东莞华南处理中心'
  ]],
  [/湖北|武汉|黄石|十堰|宜昌|襄阳|鄂州|荆门|孝感|荆州|黄冈|咸宁|随州|恩施|仙桃|湖南|长沙|株洲|湘潭|衡阳|邵阳|岳阳|常德|张家界|益阳|郴州|永州|怀化|娄底|河南|郑州|开封|洛阳|平顶山|安阳|鹤壁|新乡|焦作|濮阳|许昌|漯河|三门峡|南阳|商丘|信阳|周口|驻马店/, [
    '武汉华中处理中心', '郑州中原转运中心'
  ]],
  [/四川|成都|绵阳|自贡|攀枝花|泸州|德阳|广元|遂宁|内江|乐山|宜宾|南充|达州|雅安|阿坝|甘孜|凉山|广安|巴中|眉山|重庆|云南|昆明|曲靖|玉溪|保山|昭通|丽江|普洱|临沧|贵州|贵阳|六盘水|遵义|安顺|毕节|铜仁|西藏|拉萨|陕西|西安|铜川|宝鸡|咸阳|渭南|延安|汉中|榆林|安康|商洛|甘肃|兰州|嘉峪关|金昌|白银|天水|武威|张掖|平凉|酒泉|庆阳|定西|陇南|青海|西宁|海东|宁夏|银川|新疆|乌鲁木齐/, [
    '成都西南集散中心'
  ]]
];

/** 根据地址/省份/城市匹配就近转运中心 */
export function nearestHub(location) {
  if (!location) return pick(HUBS);
  for (const [re, hubs] of REGION_RE) {
    if (re.test(location)) return pick(hubs);
  }
  return pick(HUBS);
}

/** 从转运中心名称中提取城市名,如 "杭州转运中心" → "杭州" */
export function hubCity(hub) {
  if (!hub) return '';
  // 取开头的2-4个中文字符作为城市名
  const m = hub.match(/^([一-鿿]{2,4})/);
  return m ? m[1] : hub;
}
